...
study_data_raw <- read_csv("data_set_export.csv")
study_data_raw <- study_data_raw %>% mutate(study = as.factor(study))

# Calculating percentage weight regain with error propagation (X/Y)
...
study_data <- calculate_weight_regain(study_data_raw)
...
monte_carlo <- function(fit, dat, B = 100) {
  ...
  for (b in seq_len(B)) {
    ...
    delta_weight_mean_jitter <- delta_weight_mean_observed + rnorm(length(delta_weight_mean_observed), mean = 0, sd = digitize_sd)
...
set.seed(42)
monte_carlo_data <- monte_carlo(model, study_data_raw, B = 1000)
