...
# Fitted values back on raw scale
pred_logit <- predict(model_logit)              # on logit scale
pred_prop  <- plogis(pred_logit)        # 0..1
pred_pct   <- 100 * pred_prop           # 0..100
...
# BACK-TRANSFORMED LOGIT
study_data <- study_data %>%
  mutate(predicted_logit_bt = plogis(predicted_logit) * 100)


# Fixed effects predictions (smooth curve)
fixed_effects_grid_logit_bt <- fixed_effects_grid_logit %>%
  mutate(predicted_fixed_bt = plogis(predicted_fixed) * 100)


# Random effects predictions (study-specific)
random_effects_grid_logit_bt <- random_effects_grid_logit %>%
  mutate(predicted_random_bt = plogis(predicted_random) * 100)
...
ggplot(study_data, aes(x = wks_after_cessation, y = weight_regain_pct, color = study)) +
  ...
  # Fixed effect curve
  geom_line(data = fixed_effects_grid_logit_bt,
            aes(x = wks_after_cessation, y = predicted_fixed_bt),
  ...
  geom_line(data = random_effects_grid_logit_bt,
            aes(x = wks_after_cessation, y = predicted_random_bt, group = study, linetype = study), size = 0.8, alpha = 0.8) +
...
fit_bt <- plogis(fitted_logit) * 100               # fitted %
obs_bt <- study_data$weight_regain_pct            # observed %
resid_bt <- obs_bt - fit_bt                   # back-transformed residuals
...
plot(fitted_logit, resid_logit,
     main = "Residuals (Logit)",
     xlab = "Fitted values (logit scale)",
     ylab = "Residuals (logit scale)")
abline(h = 0, lty = 2, col = "red")

# Backtransformed logit model
plot(fit_bt, resid_bt,
     main = "Residuals (back-transformed logit model)",
     xlab = "Fitted values (% regain)",
     ylab = "Residuals (% regain)")
abline(h = 0, lty = 2, col = "red")
...
qqnorm(resid_identity, main = "Q-Q Plot (Identity)")
qqline(resid_identity, col = "red")

qqnorm(resid_logit, main = "Q-Q Plot (Logit)")
qqline(resid_logit, col = "red")

qqnorm(resid_bt, main = "Q-Q Plot (back-transformed logit model)")
qqline(resid_bt, col = "red")
